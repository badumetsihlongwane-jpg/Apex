...
PAIR_FEATS = ['Close', 'High', 'Low', 'Volume', 'RSI_14', 'MACD', 'MACD_sig', 'BBL_20_2.0', 'BBU_20_2.0', 'ATR_14', 'ROC_10', 'EMA_20', 'EMA_50']
...
def _align_tf(df_tf: pd.DataFrame, t_ref: pd.Timestamp, ctx_bars: int, pairs: List[str], cfg: OmegaConfig) -> np.ndarray:
    ...
    # Slicing up to t_ref (exclusive of any future times)
...
def fetch_timeframe_data(symbol_map: Dict[str, str], timeframe: int, count: int) -> pd.DataFrame:
    ...
    for pair, mt5_sym in symbol_map.items():
        ...
        # We start from index 1 (the previous complete candle) to avoid using the forming live candle
    ...
    # Interpolate missing quotes if any broker synchronization latency exists
...
def get_filling_type(symbol: str) -> int:
    ...
    # Use raw bitmask values (1 = FOK, 2 = IOC) to support older MT5 library versions
    if filling & 1:
...
def close_all_positions(symbol: str) -> bool:
    ...
    for pos in positions:
        # Skip positions not belonging to our strategy magic number
        ...
        for attempt in range(3):
            ...
            if result is None:
                ...
                if result.retcode == 10018:
                    ...
                    break # Market closed, no use retrying now
                else:
...
def execute_rebalance(symbol: str, target_weight: float) -> None:
    ...
    # Get current open positions on our magic number
    positions = mt5.positions_get(symbol=symbol)
    current_volume = 0.0
    if positions:
        for pos in positions:
    ...
    # Minimum trade unit is typically 0.01
    ...
    # 1. Close all open positions first to prevent duplicate hedging or nesting issues
    if not close_all_positions(symbol):
        print(f"[TRADE] Error clearing old positions for {symbol}. Rebalancing aborted.")
        return
        
    # 2. Open new position if target lot is non-zero
...
def run_live_inference():
    ...
    # 1. Load trained neural weights
    in_features = 24 # 13 indicators + 5 one-hot asset IDs + 2 yields + 1 NFP + 3 trade state features
    ...
    # Initialize optimizer for Return Probe online adaptation (Level 2)
    probe_optimizer = torch.optim.AdamW(model.return_probe.parameters(), lr=0.001)

    # Buffers for Level 1 & Level 2 online adaptation
    ...
    # Map raw symbols to MT5 symbol formats (suffixes/prefixes)
    symbol_map = {pair: get_mt5_symbol(pair) for pair in cfg.PAIRS}
    print(f"[MT5] Symbol Map: {symbol_map}")
    
    # We require 120 bars of data to correctly initialize features and handle NaN drops
    fetch_bars = 120
    last_processed_timestamp = None
    
    try:
        while True:
            # 3. Check for the arrival of a new completed 5m candle
            # Fetch latest rates for the first pair to verify timestamp
            ...
            # Execute trade check only when the completed bar timestamp moves forward
            if last_processed_timestamp is None or current_bar_time > last_processed_timestamp:
                ...
                try:
                    # 4. Fetch and align multi-timeframe OHLCV data
                    ...
                    # 4b. Fetch and align sovereign yields
                    ...
                    # 4c. Load calendar and run precomputations
                    ...
                    # Overwrite simulated trade features of the latest row with actual MT5 position stats
                    for pair in cfg.PAIRS:
                    ...
                    # Align other timeframes to reference time of the latest 5m candle
                    ...
                    for n, pair in enumerate(cfg.PAIRS):
                        ...
                        arr = np.concatenate([arr, one_hot], axis=-1)
                        forex_5m.append(arr)
                    x_5m = np.stack(forex_5m, axis=1) # [T5, N, 36]
                    ...
                    # 5. Convert to tensor and run forward pass
                    ...
                    # 5a. Level 1 Online Fast Weight adaptation (transition update)
                    ...
                    with torch.no_grad():
                        ...
                        # Apply leverage normalization to keep portfolio leverage stable during adaptation
                    ...
                    # 5b. Level 2 Consolidation (Return Probe SGD)
                    # Check if the oldest entry in queue is 1 hour old (12 bars of 5m)
                    while len(adaptive_queue) > 0:
                        oldest = adaptive_queue[0]
                        dt_min = (current_bar_time - oldest['time']).total_seconds() / 60.0
                        if dt_min >= 60.0:
                            # 1 hour has elapsed, run SGD consolidation
                    ...
                    # 6. Execute rebalancing deals on MT5 (excluding USDJPY)
                    ...
                    for n, pair in enumerate(cfg.PAIRS):
                        ...
                        # 6a. Informational News print (decision made by neural policy)
                ...
                # Print live heartbeat to console so user knows it's running
            ...
            # Poll every 10 seconds for candle transitions
            time.sleep(10)
